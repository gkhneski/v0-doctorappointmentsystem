...
CREATE TABLE IF NOT EXISTS pregnancy_episodes (
  ...
  conception_type text NULL CHECK (conception_type IN ('spontan', 'ART', 'IUI', 'operasyon', 'diger') OR conception_type IS NULL),
  sat_date date NULL,
  edd_date date NULL,
  et_date date NULL,
...
CREATE TABLE IF NOT EXISTS pregnancy_outcomes (
  ...
  delivery_type text NULL CHECK (delivery_type IN ('C/S', 'N/D') OR delivery_type IS NULL),
  baby_count int NULL,
  hospital text NULL,
...
ALTER TABLE patient_documents 
  ADD COLUMN IF NOT EXISTS pregnancy_visit_id uuid NULL REFERENCES pregnancy_visits(id) ON DELETE SET NULL;

ALTER TABLE patient_documents 
  ADD COLUMN IF NOT EXISTS pregnancy_episode_id uuid NULL REFERENCES pregnancy_episodes(id) ON DELETE SET NULL;
...
CREATE POLICY IF NOT EXISTS "Admin can view all pregnancy episodes"
  ON pregnancy_episodes FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY IF NOT EXISTS "Admin can insert pregnancy episodes"
  ON pregnancy_episodes FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "Admin can update pregnancy episodes"
  ON pregnancy_episodes FOR UPDATE
  TO authenticated
  USING (true);

CREATE POLICY IF NOT EXISTS "Admin can delete pregnancy episodes"
...
CREATE POLICY IF NOT EXISTS "Admin can view all pregnancy visits"
  ON pregnancy_visits FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY IF NOT EXISTS "Admin can insert pregnancy visits"
  ON pregnancy_visits FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "Admin can update pregnancy visits"
  ON pregnancy_visits FOR UPDATE
  TO authenticated
  USING (true);

CREATE POLICY IF NOT EXISTS "Admin can delete pregnancy visits"
...
CREATE POLICY IF NOT EXISTS "Admin can view all pregnancy outcomes"
  ON pregnancy_outcomes FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY IF NOT EXISTS "Admin can insert pregnancy outcomes"
  ON pregnancy_outcomes FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY IF NOT EXISTS "Admin can update pregnancy outcomes"
  ON pregnancy_outcomes FOR UPDATE
  TO authenticated
  USING (true);

CREATE POLICY IF NOT EXISTS "Admin can delete pregnancy outcomes"
